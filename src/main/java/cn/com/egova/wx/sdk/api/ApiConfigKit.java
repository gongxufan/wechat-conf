...
import cn.com.egova.wx.sdk.cache.DefaultAccessTokenCache;
import cn.com.egova.wx.sdk.cache.IAccessTokenCache;

/**
 * 将 ApiConfig 绑定到 ThreadLocal 的工具类，以方便在当前线程的各个地方获取 ApiConfig 对象：
 * 1：如果控制器继承自 WxMsgHandler 该过程是自动的，详细可查看 WxMsgInterceptor 与之的配合
 * 2：如果控制器继承自 WxApiHandler 该过程是自动的，详细可查看 WxApiInterceptor 与之的配合
 * 3：如果控制器没有继承自 WxMsgHandler、WxApiHandler，则需要先手动调用
