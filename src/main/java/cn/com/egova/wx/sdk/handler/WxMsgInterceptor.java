...
import cn.com.egova.wx.sdk.kit.SignatureCheckKit;
...
/**
 * Msg 拦截器
 * 1：通过 WxMsgHandler.getApiConfig() 得到 ApiConfig 对象，并将其绑定到当前线程之上(利用了 ApiConfigKit 中的 ThreadLocal 对象)
 * 2：响应开发者中心服务器配置 URL 与 Token 请求
 * 3：签名检测
 * 注意： WxMsgHandler 的继承类如果覆盖了 index 方法，则需要对该 index 方法声明该拦截器
 * 		因为子类覆盖父类方法会使父类方法配置的拦截器失效，从而失去本拦截器的功能
 */
public class WxMsgInterceptor implements Interceptor {

    private static final Log log = Log.getLog(WxMsgInterceptor.class);

    public void intercept(Invocation inv) {
        Controller controller = inv.getController();
        if (controller instanceof WxMsgHandler == false)
            throw new RuntimeException("控制器需要继承 WxMsgHandler");

        try {
            // 将 ApiConfig 对象与当前线程绑定，以便在后续操作中方便获取该对象： ApiConfigKit.getApiConfig();
            ApiConfigKit.setThreadLocalApiConfig(((WxMsgHandler) controller).getApiConfig());
