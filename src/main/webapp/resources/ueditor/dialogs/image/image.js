...
(function () {
    ...
    function initTabs() {
        ...
        var flag = parent.document.getElementById('selectThumbFlag').value;
        ...
        for(var j = 0 ; j < tabs.length ; j++){
            //选择封面打开对话框要隐藏上传按钮
            if(flag == 1 && tabs[j].getAttribute('data-content-id') == 'upload'){
                tabs[j].style = "display:none";
                continue;
            }
            if(tabs[j].getAttribute('data-content-id') == 'online'){
                setTabFocus("online");
                break;
            }
        }
        ...
        dialog.onok = function () {
            ...
            if(list) {
                ...
                if(flag != '1'){
                    ...
                    var img = list[0];
                    var _parent = parent;
                    parent.getBase64Image(img.src,function (url) {
                        _parent.document.getElementById('coverPic').setAttribute('src', url);
                        //同步左侧图文信息区
                        ($(_parent.document).find(".item-list").find("li").filter(".active").find("img"))[0].setAttribute('src', url);
                    });
                    parent.document.getElementById('selectThumbFlag').value = '0';
                    //属性数据同步
                    parent.wxEditor.articles.get($(window.parent.document).find(".item-list").find("li").filter(".active").attr("id")).thumb_media_id =img. media_id;
                    //显示图片
                    $(window.parent.document).find("#coverDiv").show();
                }
        ...
        pushData: function (list) {
            ...
            for (i = 0; i < list.length; i++) {
                if(list[i] && list[i].url) {
                    ...
                    img.setAttribute('media_id',list[i].media_id);
                    domUtils.on(img, 'load', (function(image){
                        parent.getBase64Image(image.getAttribute('_src'),function (url) {
            ...
            for (i = 0; i < lis.length; i++) {
                if (domUtils.hasClass(lis[i], 'selected')) {
                    var img = lis[i].firstChild,
                        src = img.getAttribute('_src'),media_id = img.getAttribute('media_id');
                    list.push({
                        src: src,
                        _src: src,
                        media_id:media_id,
