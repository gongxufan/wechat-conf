                    ...
                    <div class="appmsgCnt">
                        <c:forEach items="${col1}" var="news1">
                            <c:if test="${(news1)!= null && fn:length(news1) == 1}">
                                ...
                                <div  name="msg"  url=${news2.url} class="appmsg single has_first_cover" id="${news2.media_id}">
                            ...
                            <c:if test="${(news1)!= null && fn:length(news1) > 1}">
                                <div class="appmsg multi has_first_cover"  url=${news1[0].url}   name="msg" id="${news1[0].media_id}">
                                    <div class="appmsg_content">
                                        <div class="appmsg_info">
                        ...
                        <c:forEach items="${col2}" var="news3">
                            <c:if test="${(news3)!= null && fn:length(news3) == 1}">
                                <c:forEach items="${news3}" var="news4">
                                    <div  name="msg"   url=${news4.url} class="appmsg single has_first_cover" id="${news4.media_id}">
                                        <div class="appmsg_content">
                                            <div class="appmsg_info">
                ...
                <span class="page_nav_area">
                    <a class="btn page_first" href="javascript:void(0);" style="display: none;"></a>
                                <c:if test="${pageNow <= pages && pageNow > 1}">
                                    <a onclick="dialogPage('<%=request.getContextPath()%>/weixin/api/batchGetMaterial?cols=2&type=NEWS&pageNow=${pageNow-1}&count=4&view=card')" href="javascript:;" class="btn page_prev"><i class="arrow"></i></a>
                                </c:if>
                                <span class="page_num">
                                    <label>${pageNow}</label>
                                ...
                                <c:if test="${pageNow < pages}">
                                    <a onclick="dialogPage('<%=request.getContextPath()%>/weixin/api/batchGetMaterial?cols=2&type=NEWS&pageNow=${pageNow+1}&count=4&view=card');" href="javascript:;" class="btn page_next"><i class="arrow"></i></a>
                                </c:if>
